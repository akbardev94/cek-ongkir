    ...
    <div className="max-w-lg border shadow-lg bg-slate-50 mx-auto my-7 p-5 rounded-lg">
      <form className="mt-5">
        ...
        <div className="w-full bg-slate-200 p-2">
          <p className="text-2xl text-orange-900 font-semibold mt-1 mb-1">
            Kota Asal
          </p>
        </div>
        ...
        <select
          name="prov1"
          id="prov1"
          onChange={(e: { target: { value: string; }; }) => getCity1(e.target.value)}
          className="input-section"
        >
          <option value="">-- Pilih Provinsi --</option>
          {provinces.map((item: { province_id: any; province: any; }) => (
        ...
        <select
          name="kot1"
          id="kot1"
          onChange={(e: { target: { value: string; }; }) => valAsal(e.target.value)}
          className="input-section"
        >
          <option value="">-- Pilih kota --</option>
          {cities1.map((item: { city_id: any; type: any; city_name: any; }) => (
        ...
        <select
          name="prov2"
          id="prov2"
          onChange={(e: { target: { value: string; }; }) => getCity2(e.target.value)}
          className="input-section"
        >
          <option value="">-- Pilih provinsi --</option>
          {provinces.map((item: { province_id: any; province: any; }) => (
        ...
        <select
          name="kot2"
          id="kot2"
          onChange={(e: { target: { value: string; }; }) => valTujuan(e.target.value)}
          className="w-full h-9 border border-slate-500 rounded-lg mb-1 pl-2"
        >
          <option value="">-- Pilih Kota --</option>
          {cities2.map((item: { city_id: any; type: any; city_name: any; }) => (
            <option value={item.city_id} key={item.city_id}>
              {item.type} {item.city_name}
            </option>
          ))}
        ...
        <input
          ...
          name="berat"
          id="berat"
          onChange={(e: { target: { value: string; }; }) => valBerat(e.target.value)}
        ...
        <select
          id="kurir"
          name="kurir"
          onChange={(e: { target: { value: string; }; }) => valKurir(e.target.value)}
      ...
      <div className="mt-3">
        <div>
          {costList.map((item: { service: any; cost: { value: number; }[]; }, index: any) => (
            <div
              key={index}
